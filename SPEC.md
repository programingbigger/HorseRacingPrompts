...
| **当日収集モード** | 「当日収集して」「当日情報を取ってきて」 | 開発中の旨をアナウンス |
...
URL から race_id を抽出し出走馬を特定
  ↓
Info/static/ のキャッシュを参照して評価軸でスコアリング
  - 距離・間隔適性（高）
  - 前走ペース・展開の恩恵（高）
  - コース状態との適合（高）
  - 騎手の当コース成績（中）
...
#### TODO-04：当日収集モードの実装（skill②）

**背景：** 当日の馬場傾向・天気・展開予測などのリアルタイム情報収集が未実装。

**実装内容：**
- 当日の馬場状態（良・稍重・重・不良）をリアルタイム取得
- 天気情報との連携
- 展開予測（逃げ・先行・差し・追込の有利不利）
