...
オッズのリアルタイム取得は行わない（当日収集機能の実装後に対応予定）。

参照する情報：

| 情報 | 参照先 |
|---|---|
| 馬の距離適性・前走展開・体重ベスト値 | `Info/static/馬情報/{馬ID}.md` |
| 騎手の当コース成績 | `Info/static/騎手/{騎手ID}.md` |
| 競馬場のコース状態・内外有利 | `Info/static/競馬場/{中央or地方}/{競馬場名}.md` |
| コース別脚質傾向 | `Info/static/その他/コース別脚質傾向/{コース}.md` |
| 特定重賞の過去傾向 | `Info/static/その他/{レース名}/` |

キャッシュが存在しない馬・騎手の情報は「キャッシュなし」と明記し、低重み付けで扱う。
